    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-0.5">Requested: {r.quantity ?? '—'} · {new Date(r.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</p>
        </div>
      ...
      {r.status === 'confirmed' && (
        <div className="mt-2 rounded-xl border border-green-200 bg-green-50 p-3 text-sm">
          <p className="font-bold text-green-800">Confirmed by supplier ✓</p>
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-green-700 text-[13px]">
            <span>Qty: <strong>{r.confirmed_qty}</strong></span>
            <span>Price: <strong>{money(r.confirmed_price_cents, r.currency_code)}</strong></span>
            <span>Delivery: <strong>{r.delivery_time || 'TBC'}</strong></span>
          </div>
          {r.supplier_note && <p className="text-xs text-green-700 mt-1">{r.supplier_note}</p>}
        </div>
      )}
      {r.status === 'declined' && <p className="text-sm text-rose-600 mt-2">The supplier declined this request{r.supplier_note ? `: ${r.supplier_note}` : '.'}</p>}
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        ...
        {(r.status === 'requested' || r.status === 'confirmed') && (
          <button onClick={() => act('cancel')} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 text-gray-500 px-3.5 py-2 text-sm font-bold hover:bg-gray-50"><X className="w-4 h-4" /> Cancel</button>
        )}
        {r.status === 'paid' && <span className="inline-flex items-center gap-1.5 text-sm font-bold text-emerald-700"><Check className="w-4 h-4" /> Payment confirmed — the supplier will arrange delivery.</span>}
      </div>
    </div>
  )
}
