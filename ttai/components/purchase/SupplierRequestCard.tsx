  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-[12px] text-gray-500">
        <span>Wants: <strong className="text-gray-700">{r.quantity ?? '—'}</strong></span>
      ...
      {r.status === 'confirmed' && (
        <div className="mt-2 text-xs text-green-700 bg-green-50 rounded-lg px-3 py-2">
          Confirmed: <strong>{r.confirmed_qty}</strong> · {money(r.confirmed_price_cents, r.currency_code)} · {r.delivery_time || 'delivery TBC'}{r.supplier_note ? ` — ${r.supplier_note}` : ''}
        </div>
      )}

      <div className="flex flex-wrap gap-2 mt-3">
        {r.buyer_email && <a href={`mailto:${r.buyer_email}`} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-bold text-gray-600 hover:bg-gray-50"><Mail className="w-3.5 h-3.5" /> Email</a>}
        {wa && <a href={wa} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-green-200 text-green-600 px-3 py-1.5 text-xs font-bold hover:bg-green-50"><MessageCircle className="w-3.5 h-3.5" /> WhatsApp</a>}
        {r.status === 'requested' && (
          <button onClick={() => setOpen((o) => !o)} className="inline-flex items-center gap-1.5 rounded-lg bg-[#0B1F4D] text-white px-3.5 py-1.5 text-xs font-bold hover:bg-[#162d6e]"><Check className="w-3.5 h-3.5" /> Confirm</button>
        )}
        {r.status === 'requested' && (
          <button onClick={() => act('decline')} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 text-rose-600 px-3.5 py-1.5 text-xs font-bold hover:bg-rose-50"><X className="w-3.5 h-3.5" /> Decline</button>
        )}
      </div>

      {open && r.status === 'requested' && (
        <div className="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-3 grid sm:grid-cols-2 gap-2.5">
          <label className="block"><span className="text-[11px] font-bold text-gray-500">Confirmed quantity</span><input className={inp} value={form.confirmedQty} onChange={(e) => set('confirmedQty', e.target.value)} /></label>
          <label className="block"><span className="text-[11px] font-bold text-gray-500">Final price</span>
            <div className="flex gap-1.5">
              ...
              <input className={inp} type="number" step="0.01" value={form.price} onChange={(e) => set('price', e.target.value)} placeholder="total" />
            </div></label>
          <label className="block"><span className="text-[11px] font-bold text-gray-500">Delivery time</span><input className={inp} value={form.deliveryTime} onChange={(e) => set('deliveryTime', e.target.value)} placeholder="e.g. 5–7 days" /></label>
          <label className="block"><span className="text-[11px] font-bold text-gray-500">Note</span><input className={inp} value={form.note} onChange={(e) => set('note', e.target.value)} /></label>
