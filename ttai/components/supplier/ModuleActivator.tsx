  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-[#0B1F4D]/10 bg-[#0B1F4D]/[0.03] px-4 py-3 text-[12.5px] text-gray-600 leading-snug">
        <span className="font-bold text-[#0B1F4D]">One company, many channels.</span> Marketplace sellers can <strong>connect to the Outlet Zone instantly</strong> to clear stock &amp; full-pack lots. Outlet sellers need a quick <strong>verification</strong> to also sell new products in the Marketplace.
      </div>
            ...
            <div key={m.key} className={`rounded-2xl border p-5 transition-all ${on ? 'border-[#0B1F4D] bg-[#0B1F4D]/[0.03]' : gated ? 'border-amber-200 bg-amber-50/30' : 'border-gray-200'}`}>
              <div className="flex items-start justify-between gap-2">
                ...
                {on
                  ...
                  : gated
                  ? <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 text-amber-700 text-[11px] font-bold px-2 py-0.5"><Lock className="w-3 h-3" /> Verification</span>
              ...
              <div className="flex items-center gap-3 mt-3">
                {on ? (
                  ...
                  wasRequested ? (
                    <span className="inline-flex items-center gap-1.5 rounded-lg bg-green-100 text-green-700 px-4 py-2 text-xs font-bold"><Check className="w-3.5 h-3.5" /> Access requested</span>
                  ) : (
                    <button onClick={() => requestAccess(m.key)} disabled={busy === m.key}
                      ...
                      {busy === m.key ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Lock className="w-3.5 h-3.5" />} Request access
                    </button>
                  )
                ) : (
