    ...
    return (
      <div className="rounded-2xl border border-green-200 bg-green-50/60 p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-green-700">{t("Our team manages your professional online catalogue. Send updated Excel files any time.")}</p>
        </div>
      </div>
    )
  }
    ...
    <div className="rounded-2xl border border-[#0B1F4D]/15 bg-gradient-to-br from-[#0B1F4D]/[0.03] to-[#F5A623]/[0.04] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{t("Let")} <strong>TTAIEMA build &amp; maintain your professional catalogue</strong> {t("from your Excel files — featured products, category banners and brands. (Paid service)")}</p>
        </div>
      </div>
      {state === 'done' ? (
        <span className="inline-flex items-center gap-1.5 rounded-xl bg-green-100 text-green-700 px-4 py-2.5 text-sm font-bold flex-shrink-0"><Check className="w-4 h-4" /> {t("Request sent")}</span>
      ) : (
        <button onClick={request} disabled={state === 'sending'}
          ...
          {state === 'sending' ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4" />} {t("Request catalogue service")}
        </button>
      )}
    </div>
  )
