  ...
  return (
    <div className="rounded-2xl border border-[#0B1F4D]/15 bg-gradient-to-br from-[#0B1F4D]/[0.03] to-indigo-100/30 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="font-extrabold text-[#0B1F4D]">{t("Want TTAIEMA to handle your deals?")}</p>
          <p className="text-sm text-gray-500 mt-0.5">{t("Let our Marketplace team")} <strong>coordinate buyer communication, confirm stock &amp; price, and arrange dropshipping/logistics</strong> {t("on your behalf. (Optional service)")}</p>
        </div>
      </div>
      {state === 'done' ? (
        ...
        <button onClick={request} disabled={state === 'sending'}
          ...
          {state === 'sending' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Handshake className="w-4 h-4" />} {t("Request managed service")}
        </button>
      )}
    </div>
  )
