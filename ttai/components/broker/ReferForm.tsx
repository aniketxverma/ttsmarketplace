...
export function ReferForm() {
  ...
  if (done) {
    return (
      <div className="rounded-2xl border border-green-100 bg-white p-6 text-center">
        ...
        <p className="mt-2 font-extrabold text-[#0B1F4D]">Reference registered · +{POINTS_PER_REFERRAL} points</p>
        <p className="text-sm text-gray-500 mt-1">This company is now permanently linked to your Broker ID.</p>
        <button onClick={() => setDone(false)} className="mt-3 text-sm font-bold text-[#0B1F4D] underline">Register another</button>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-2xl border border-gray-200 bg-white p-6">
      <h3 className="text-lg font-extrabold text-[#0B1F4D]">Register a reference</h3>
      <p className="text-sm text-gray-500 mt-0.5">Add a company to your protected network · +{POINTS_PER_REFERRAL} connection points each.</p>
      ...
      <div className="grid sm:grid-cols-2 gap-3 mt-4">
        <label className="block sm:col-span-2"><span className={lbl}>Company name *</span><input name="companyName" required className={input} /></label>
        <label className="block"><span className={lbl}>Contact name</span><input name="contactName" className={input} /></label>
        <label className="block"><span className={lbl}>Country</span><input name="countryName" className={input} /></label>
        <label className="block"><span className={lbl}>Email</span><input name="contactEmail" type="email" className={input} /></label>
        <label className="block"><span className={lbl}>Phone / WhatsApp</span><input name="contactPhone" className={input} /></label>
        <label className="block"><span className={lbl}>Category / sector</span><input name="category" className={input} placeholder="e.g. Electronics" /></label>
