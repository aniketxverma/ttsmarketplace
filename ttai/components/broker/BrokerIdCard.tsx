...
export function BrokerIdCard({ code }: { code: string }) {
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-[#0B1F4D] to-[#13306e] text-white p-5 relative overflow-hidden">
      ...
      <p className="text-[11px] font-bold uppercase tracking-widest text-white/60 flex items-center gap-1.5"><BadgeCheck className="w-3.5 h-3.5" /> Your Broker ID</p>
      <p className="text-2xl font-black tracking-wider mt-1.5">{code}</p>
      <p className="text-[11px] text-white/60 mt-1">Every company you register stays linked to this ID — permanently.</p>
