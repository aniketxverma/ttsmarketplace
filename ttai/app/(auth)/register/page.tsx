...
const ABOUT_PROMPTS: Record<string, { bio: string; products: string }> = {
  buyer: {
    bio:      'Describe your company and what you are looking to import or purchase…',
    products: 'List the specific products or brands you are sourcing (e.g. Pepsi, cleaning sprays, electronics)…',
  },
  business_client: {
    bio:      'Describe your business, your purchasing needs and target volumes…',
    products: 'List the key product categories or specific items you need to procure…',
  },
  supplier: {
    bio:      'Describe your company, manufacturing capabilities and what makes you stand out…',
    products: 'List your main products, brands or services (e.g. Rozil cleaning range, private-label detergents)…',
  },
  broker: {
    bio:      'Describe your brokerage expertise, network and how you create value for clients…',
    products: 'List the sectors, product categories and regions you specialize in…',
  },
}
...
type FormData = {
  ...
  // Step 2 — Company
  companyName: string; businessType: string; continent: string; countryName: string
  city: string; category: string; annualTurnover: string; websiteUrl: string
  // Step 3 — About
...
function Input({
  type = 'text', value, onChange, placeholder, required = true,
}: {
...
function ReviewRow({ label, value, private: priv }: { label: string; value: string; private?: boolean }) {
  ...
  return (
    <div className="flex gap-3 px-4 py-2.5">
      ...
      <div className="flex-1 flex items-start gap-1.5">
        ...
        {priv && (
          <span className="flex-shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-full mt-0.5">
...
export default function RegisterPage() {
  const [step, setStep]       = useState(0)   // 0 = role picker
  ...
  function pickRole(r: string) {
    set('role', r)
    set('businessType', '')   // reset role-specific field
    setStep(1)
  }
  ...
  const step1Valid =
    form.fullName.trim() && form.email.includes('@') && form.phone.trim() && form.password.length >= 8

  const step2Valid =
    form.companyName.trim() && form.businessType && form.continent &&
    form.countryName.trim() && form.city.trim() && form.category
  ...
  async function submit() {
    ...
    if (data.user) {
      await supabase.from('profiles').update({
        ...
        city:             form.city,
        category:         form.category,
        annual_turnover:  form.annualTurnover || null,
        website_url:      form.websiteUrl     || null,
        bio:              form.bio,
        products_offered: form.productsOffered || null,
      }).eq('id', data.user.id)
    }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ROLES.map((r) => (
            <button
              ...
              className="group relative bg-white rounded-2xl border-2 border-gray-100 p-5 text-left
                hover:border-[#0B1F4D] hover:shadow-lg transition-all duration-200 overflow-hidden"
            >
              {/* Gradient stripe */}
              ...
              <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${r.color} flex items-center justify-center text-white mb-3 shadow-md group-hover:scale-105 transition-transform`}>
                {r.icon}
              </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-[#0B1F4D] to-[#1a3580] px-6 py-5">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className={`inline-flex items-center gap-1.5 bg-gradient-to-r ${roleObj?.color} text-white text-xs font-bold px-3 py-1 rounded-full`}>
            {roleObj?.label}
          </div>
        </div>
            ...
            <div key={s.num} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold transition-all
                  ${step > s.num ? 'bg-[#F5A623] text-white'
                    : step === s.num ? 'bg-white text-[#0B1F4D]'
              ...
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-1.5 mb-4 transition-all duration-500 ${step > s.num ? 'bg-[#F5A623]' : 'bg-white/20'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">Login credentials and contact info</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label text="Email *" />
                <Input type="email" value={form.email} onChange={(v) => set('email', v)} placeholder="you@company.com" />
              </div>
              <div className="space-y-1.5">
                <Label text="Phone / WhatsApp *" />
                <Input type="tel" value={form.phone} onChange={(v) => set('phone', v)} placeholder="+34 600 000 000" />
              </div>
              <div className="sm:col-span-2 space-y-1.5">
                <Label text="Password * (min 8 characters)" />
                <Input type="password" value={form.password} onChange={(v) => set('password', v)} placeholder="Min 8 characters" />
              </div>
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.companyName} onChange={(v) => set('companyName', v)} placeholder="Acme Trading Co." />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  options={BUSINESS_TYPES[role] ?? BUSINESS_TYPES.buyer}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select value={form.continent} onChange={(v) => set('continent', v)} options={CONTINENTS} placeholder="Select continent" />
                </div>
                <div className="space-y-1.5">
                  <Label text="Country *" />
                  <Input value={form.countryName} onChange={(v) => set('countryName', v)} placeholder="Spain, UAE…" />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <Label text="City *" />
                <Input value={form.city} onChange={(v) => set('city', v)} placeholder="Madrid, Dubai…" />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  options={CATEGORIES}
                  placeholder="e.g. Electronics, Food, Logistics"
                />
              </div>

              {/* Annual turnover (supplier/broker only) */}
              {showTurnover && (
                <div className="space-y-1.5">
                  ...
                  <p className="text-[11px] text-gray-400 flex items-center gap-1 mt-1">
                    ...
                    Stored privately — not shown on your public profile
                  </p>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                ...
                <Input type="url" value={form.websiteUrl} onChange={(v) => set('websiteUrl', v)} placeholder="https://yourcompany.com" required={false} />
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-5">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-0.5">This appears on your public profile</p>
            </div>
        ...
        {step === 4 && (
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border border-gray-100 overflow-hidden divide-y divide-gray-50 text-sm">
              ...
              {showTurnover && <ReviewRow label="Turnover" value={form.annualTurnover} private />}
            ...
            <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-4">
              ...
              {[
                ...
                'Full access granted — your profile goes live',
              ].map((s, i) => (
                <div key={i} className="flex items-center gap-2.5 py-1">
                  <span className="w-5 h-5 rounded-full bg-amber-200 text-amber-800 text-[10px] font-extrabold flex items-center justify-center flex-shrink-0">
                    {i + 1}
                  </span>
            ...
            <p className="text-center text-xs text-gray-400">
              ...
              <span className="text-[#0B1F4D] font-semibold hover:underline cursor-pointer">Terms</span>
              {' '}&{' '}
              <span className="text-[#0B1F4D] font-semibold hover:underline cursor-pointer">Privacy Policy</span>
            </p>
          </div>
        )}
      </div>
