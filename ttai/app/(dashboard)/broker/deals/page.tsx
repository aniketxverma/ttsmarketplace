...
export default async function BrokerDealsPage() {
  ...
  const tt = await localizeUI(["Commission history and the optional TTAIEMA invoicing service.", "Total deals", "Commission logged", "No deals yet", "Submit your first deal on the right.", "Value:", "Commission:", "TTAIEMA invoicing"], getLocale())
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">{tt("Commission history and the optional TTAIEMA invoicing service.")}</p>
      </div>

      <div className="grid lg:grid-cols-[1fr_420px] gap-6 items-start">
        <div>
          <div className="rounded-xl border bg-card p-4 mb-4 flex items-center justify-between">
            <div><p className="text-2xl font-bold text-[#0B1F4D]">{deals.length}</p><p className="text-xs text-gray-400">{tt("Total deals")}</p></div>
            <div className="text-right"><p className="text-2xl font-bold text-green-600">{money(totalCommission)}</p><p className="text-xs text-gray-400">{tt("Commission logged")}</p></div>
          </div>

          {deals.length === 0 ? (
            <div className="text-center py-14 bg-white rounded-2xl border border-gray-100">
              <p className="text-gray-600 font-semibold">{tt("No deals yet")}</p>
              <p className="text-gray-400 text-sm mt-1">{tt("Submit your first deal on the right.")}</p>
            </div>
          ) : (
              ...
              {deals.map((d) => (
                <div key={d.id} className="rounded-2xl border border-gray-200 bg-white p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-gray-500">
                    {d.price_cents != null && <span>{tt("Value:")} <strong className="text-gray-700">{money(d.price_cents, d.currency_code)}</strong></span>}
                    {d.commission_pct != null && <span>{tt("Commission:")} <strong className="text-green-600">{d.commission_pct}% · {money(d.commission_cents, d.currency_code)}</strong></span>}
                    {d.needs_invoicing && <span className="text-violet-600 font-bold">{tt("TTAIEMA invoicing")}</span>}
