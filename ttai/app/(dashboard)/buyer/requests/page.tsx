...
export default async function BuyerRequestsPage() {
  ...
  const tt = await localizeUI(["My Purchase Requests", "No purchase requests yet", "Use “Want to Buy” on a B2B product to send a request."], getLocale())
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold">{tt("My Purchase Requests")}</h1>
      ...
      {requests.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-400 text-sm mt-1">{tt("Use “Want to Buy” on a B2B product to send a request.")}</p>
        </div>
      ) : (
