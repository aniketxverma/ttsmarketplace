...
export default async function BuyerDashboardPage() {
  ...
  const tt = await localizeUI(["My Orders", "Messages", "Total Orders", "Total Spent", "Active Orders", "Order ID", "Amount", "Status", "Date", "View", "No orders yet.", "Browse marketplace"], getLocale())
  await requireAuth()
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-xl border bg-card p-4">
          <p className="text-xs text-muted-foreground">{tt("Total Orders")}</p>
          <p className="text-2xl font-bold mt-1">{orders?.length ?? 0}</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <p className="text-xs text-muted-foreground">{tt("Total Spent")}</p>
          <p className="text-2xl font-bold mt-1">{formatCents(totalSpent, 'EUR')}</p>
        </div>
        <div className="rounded-xl border bg-card p-4">
          <p className="text-xs text-muted-foreground">{tt("Active Orders")}</p>
      ...
      {orders?.length ? (
        ...
        <div className="rounded-xl border bg-card p-8 text-center">
          <p className="text-sm text-muted-foreground">{tt("No orders yet.")}</p>
          <Link href="/marketplace" className="mt-3 inline-block text-sm text-primary hover:underline">{tt("Browse marketplace")}</Link>
        </div>
      )}
    </div>
  )
