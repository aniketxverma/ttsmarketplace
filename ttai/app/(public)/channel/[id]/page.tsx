...
export default async function ChannelPage({ params }: { params: { id: string } }) {
  ...
  const tt = await localizeUI(["followers", "Channel", "posts", "Follow WhatsApp Channel", "No posts yet — follow to be the first to know.", "Visit", "store"], getLocale())
  const supabase = createClient()
        ...
        <div className="max-w-2xl mx-auto px-2 sm:px-4 h-14 flex items-center gap-2 text-white">
          ...
          <div className="flex-1 min-w-0 leading-tight">
            ...
            <p className="text-white/70 text-[12px] truncate">
              {channel.member_count.toLocaleString()} {tt("followers")}
            </p>
          </div>
      ...
      <div className="bg-white border-b border-black/5">
        <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col items-center text-center">
          ...
          <p className="text-[13px] text-[#667781] mt-0.5">
            {tt("Channel")}{supplier?.trade_name ? ` · ${supplier.trade_name}` : ''}
          </p>
          {channel.description && (
          ...
          <p className="text-[13px] text-[#667781] mt-1.5">
            {channel.member_count.toLocaleString()} followers · {channel.post_count.toLocaleString()} {tt("posts")}
          </p>
          ...
          {posts.length === 0 ? (
            <div className="flex justify-center mt-6">
              <span className="bg-[#ffffff] text-[#54656f] text-[12.5px] px-3 py-1.5 rounded-lg shadow-sm">
                {tt("No posts yet — follow to be the first to know.")}
              </span>
            </div>
          ) : (
              ...
              {supplier?.brand_slug && (
                <div className="pt-6 flex justify-center">
                  <Link href={`/brand/${supplier.brand_slug}`}
                    ...
                    {tt("Visit")} {supplier.trade_name ?? ''} {tt("store")}
