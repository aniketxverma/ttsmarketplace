...
export default async function ProductPage({ params, searchParams }: { params: { slug: string }; searchParams: { shop?: string } }) {
  ...
  const tt = await localizeUI(["Catalogue (PDF)", "Watch video", "WhatsApp", "Specifications", "On request"], getLocale())
    ...
    <div className="min-h-screen bg-[#F7F8FA]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-8">
              ...
              <div className="flex flex-wrap gap-2">
                {product.catalogue_url && (
                  <a href={product.catalogue_url} target="_blank" rel="noopener noreferrer"
                    ...
                    <Package className="w-3.5 h-3.5" /> {tt("Catalogue (PDF)")}
                  </a>
                )}
                {product.video_url && (
                  <a href={product.video_url} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0B1F4D] border border-gray-200 hover:border-[#0B1F4D] bg-white px-3 py-2 rounded-lg transition-all">
                    <ChevronLeft className="w-3.5 h-3.5 rotate-180" /> {tt("Watch video")}
                  </a>
                )}
              </div>
            )}
        ...
        {(() => {
          ...
          return (
            <div className="mt-14">
              <h2 className="text-lg font-extrabold text-[#0B1F4D] mb-5">{tt("Specifications")}</h2>
              ...
              {more.map((p: any) => (
                <Link key={p.id} href={`/product/${p.slug ?? p.id}${houseBrand ? '?shop=b2b' : ''}`}
                  ...
                  <div className="p-2.5">
                    ...
                    <p className="text-xs font-extrabold text-[#0B1F4D]">{p.price_cents > 0 ? fmt(p.price_cents, p.currency_code) : <span className="text-violet-700">{tt("On request")}</span>}</p>
                  </div>
                </Link>
              ))}
            </div>
