    ...
    <div className="min-h-screen bg-[#F7F8FA]">
      ...
      <div className="border-b bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-8 h-11 flex items-center gap-2 text-sm text-gray-400">
          <Link href="/marketplace" className="hover:text-[#0B1F4D] transition-colors">Marketplace</Link>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-baseline gap-3 flex-wrap mb-4">
                ...
                <span className="text-sm text-gray-400 font-medium">/ unidad</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#F7F8FA] rounded-xl p-3">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">Pedido mínimo</p>
                  <p className="text-lg font-extrabold text-[#0B1F4D]">{product.min_order_qty} <span className="text-sm font-semibold text-gray-500">uds</span></p>
                </div>
                <div className="bg-[#F7F8FA] rounded-xl p-3">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-0.5">Stock disponible</p>
                  <div className="flex items-center gap-1.5">
                    ...
                    <p className="text-lg font-extrabold text-[#0B1F4D]">
                      ...
                      <span className="text-sm font-semibold text-gray-500"> uds</span>
                    </p>
                  </div>
                </div>
              </div>
            ...
            {product.description && (
              <div>
                <h2 className="text-sm font-extrabold text-gray-700 uppercase tracking-wide mb-2">Descripción</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {waHref && (
                <a href={waHref} target="_blank" rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2.5 bg-green-500 hover:bg-green-400 text-white py-3.5 px-6 rounded-xl font-bold text-base shadow-md hover:shadow-lg transition-all">
              ...
              {supplier?.phone && (
                <a href={`tel:${supplier.phone}`}
                  className="flex items-center justify-center gap-2 bg-white border border-gray-200 hover:border-[#0B1F4D] text-[#0B1F4D] py-3.5 px-5 rounded-xl font-bold text-base transition-all">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    {supplier.years_experience > 0 && (
                      <span className="text-[10px] text-gray-500 flex items-center gap-0.5">
                        <Package className="w-3 h-3" />{supplier.years_experience} años
                      </span>
                    )}
                    {supplier.countries_served > 0 && (
                      <span className="text-[10px] text-gray-500 flex items-center gap-0.5">
                        <Users className="w-3 h-3" />{supplier.countries_served} países
                      </span>
                    )}
                  </div>
                </div>
        ...
        {more.length > 0 && (
          <div className="mt-14">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-extrabold text-[#0B1F4D]">Más productos de {supplier?.trade_name}</h2>
              {supplier?.brand_slug && (
                <Link href={`/brand/${supplier.brand_slug}`}
