...
export default async function TradeBoardPage({ searchParams }: { searchParams: { kind?: string } }) {
  ...
  const tt = await localizeUI(["Outlet Zone", "Trade Board", "No posts yet", "Be the first — post a buying request or selling opportunity on the right.", "Qty:", "Contact", "WhatsApp"], getLocale())
    ...
    <div className="min-h-screen bg-[#F4F6FB] overflow-x-hidden">
      <div className="bg-gradient-to-br from-[#1a1207] via-[#2a1c0c] to-[#0a0e1a] text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-8 py-10">
          <Link href="/outlet" className="inline-flex items-center gap-1.5 text-xs font-bold text-white/70 hover:text-white mb-3"><ArrowLeft className="w-3.5 h-3.5" /> {tt("Outlet Zone")}</Link>
          <h1 className="text-3xl sm:text-4xl font-black">{tt("Trade Board")}</h1>
        ...
        <div>
          ...
          {rows.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-700 font-bold text-lg">{tt("No posts yet")}</p>
              <p className="text-gray-400 text-sm mt-1">{tt("Be the first — post a buying request or selling opportunity on the right.")}</p>
            </div>
          ) : (
                ...
                return (
                  <div key={r.id} className="rounded-2xl bg-white border border-gray-200 p-4 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-3 text-[11px] text-gray-500">
                      ...
                      {r.quantity && <span>{tt("Qty:")} {r.quantity}</span>}
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {r.contact_email && <a href={`mailto:${r.contact_email}`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#0B1F4D] text-white px-3.5 py-1.5 text-xs font-bold hover:bg-[#162d6e]"><Mail className="w-3.5 h-3.5" /> {tt("Contact")}</a>}
                      {wa && <a href={wa} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-green-200 text-green-600 px-3.5 py-1.5 text-xs font-bold hover:bg-green-50"><MessageCircle className="w-3.5 h-3.5" /> {tt("WhatsApp")}</a>}
                    </div>
                  </div>
                )
              })}
