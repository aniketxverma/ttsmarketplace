    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <section className="bg-gradient-to-br from-[#0B1F4D] to-[#162d6e] text-white">
        <div className="container mx-auto max-w-5xl px-4 py-14 text-center">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F5A623] text-[#0B1F4D] text-[11px] font-extrabold px-3 py-1 mb-4">
            <Truck className="w-3.5 h-3.5" /> TTAIEMA TRADE HUB
          </span>
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight">Our own goods — sold two ways</h1>
          <p className="text-blue-100/80 mt-3 max-w-2xl mx-auto">
            The Trade Hub is <span className="font-semibold text-white">TTAIEMA&apos;s own brands</span> (Bullz, Hudarom Café, Rozil and more),
      ...
      <section className="container mx-auto max-w-5xl px-4 -mt-8 pb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {cards.map((c) => (
            <Link key={c.key} href={c.href}
              ...
              {c.highlight && (
                <span className="absolute top-4 right-4 z-10 rounded-full bg-[#F5A623] text-[#0B1F4D] text-[10px] font-extrabold px-2.5 py-1">Recommended for you</span>
              )}
              ...
              <div className="p-7">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <h2 className="text-xl font-extrabold text-[#0B1F4D]">{c.title}</h2>
                </div>
                <span className="inline-block rounded-full bg-gray-100 text-gray-600 text-[11px] font-bold px-2 py-0.5 mb-3">{c.tag}</span>
                ...
                <span className="mt-6 inline-flex items-center gap-1.5 rounded-xl bg-[#0B1F4D] text-white px-5 py-3 text-sm font-bold group-hover:bg-[#162d6e] transition-colors">
                  {c.cta} <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
                </span>
              </div>
            </Link>
          ))}
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-gray-500">
          <span className="inline-flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-[#0B1F4D]" /> Fulfilled by TTAIEMA</span>
          <span className="inline-flex items-center gap-1.5"><Store className="w-4 h-4 text-[#0B1F4D]" /> One supplier, dual pricing</span>
          <span className="inline-flex items-center gap-1.5"><Truck className="w-4 h-4 text-[#0B1F4D]" /> Drop-shipping ready</span>
        </div>
      </section>
    </div>
  )
